...
data.groupby('Home Ownership').count().show() # HaveMortgage needs to get fixed as Home Mortgage

data = data.withColumn("Home Ownership", f.when(data["Home Ownership"] == 'HaveMortgage', 'Home Mortgage').otherwise(data["Home Ownership"]))

data = data.withColumn('Home Ownership', f.lower(data['Home Ownership']))

data.groupby('Loan Status').count().show() # Loan Status is fine, going to lowercase it

data = data.withColumn('Loan Status', f.lower(data['Loan Status']))

data.groupby('Term').count().show() # Going to remove 'Term' from 'Long Term' and 'Short Term'

data = data.withColumn('Term', f.lower(data['Term']))

data = data.withColumn('Term', f.regexp_extract(data['Term'], '^[a-zA-Z]+', 0))

data.groupby('Years in Current Job').count().show() # Change n/a to Unknown

data = data.withColumn('Years in Current Job', f.when(data['Years in Current Job'] == 'n/a', 'unknown').otherwise(data['Years in Current Job']))

data.groupby('Purpose').count().show() # Many values in Purpose need to get fixed

data = data.withColumn('Purpose', f.regexp_replace('Purpose', '_', ' '))
data = data.withColumn('Purpose', f.lower(data['Purpose']))

data = data.withColumn('Purpose', f.when(data['Purpose'] == 'buy a car', 'buy car').otherwise(data['Purpose']))

data = data.withColumn('Purpose', f.when(data['Purpose'] == 'take a trip', 'take trip').otherwise(data['Purpose']))
...
# Ensure datatype of Bankruptcies is of int type
data = data.withColumn("Bankruptcies", data["Bankruptcies"].cast(IntegerType()))
